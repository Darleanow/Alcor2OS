 ...
 */
static void gr__errf(char *buf, size_t cap, FILE *fallback,
                     const char *fmt, ...)
{
    char tmp[512];
    va_list args;
...
{
    if (spec->program && *spec->program) return spec->program;
...
static const gr_opt *gr__by_short(const gr_opt *opts, char key)
{
    for (; opts->kind != GR_KIND_END; opts++)
        if (opts->short_name && opts->short_name == key) return opts;
    return NULL;
}
 ...
 */
static const gr_opt *gr__by_long(const gr_opt *opts,
                                  const char *name, size_t len)
{
    for (; opts->kind != GR_KIND_END; opts++) {
        if (opts->long_name &&
            strncmp(opts->long_name, name, len) == 0 &&
...
{
    return o->kind == GR_KIND_STR  ||
           o->kind == GR_KIND_INT  ||
           o->kind == GR_KIND_UINT ||
           o->kind == GR_KIND_FLOAT;
}
...
{
    int *p = (int *)o->storage;
    if (!p) { gr__errf(buf, cap, stderr, "internal: null storage for flag"); return GR_ERR; }
    *p = 1;
    return GR_OK;
}
...
{
    int *p = (int *)o->storage;
    if (!p) { gr__errf(buf, cap, stderr, "internal: null storage for count"); return GR_ERR; }
    (*p)++;
    return GR_OK;
}
 ...
 */
static int gr__parse_int(const char *text, long *out,
                          char *buf, size_t cap, const char *label)
{
    char *end = NULL;
    long  val;
 ...
 */
static int gr__parse_uint(const char *text, unsigned long *out,
                           char *buf, size_t cap, const char *label)
{
    char         *end = NULL;
    unsigned long val;
 ...
 * @param label  Option label for error messages.
 */
static int gr__parse_float(const char *text, double *out,
                            char *buf, size_t cap, const char *label)
{
    const char *p   = text;
    double      acc = 0.0;
    ...
    for (; *p >= '0' && *p <= '9'; p++) { acc = acc * 10.0 + (*p - '0'); saw = 1; }

    if (*p == '.') {
        p++;
        frac = 1.0;
        for (; *p >= '0' && *p <= '9'; p++) {
            saw = 1; frac *= 0.1; acc += (*p - '0') * frac;
        }
    }
    if (!saw) {
    ...
    if (*p == 'e' || *p == 'E') {
        ...
        exp *= esig;
        for (; exp > 0; exp--) acc *= 10.0;
        for (; exp < 0; exp++) acc *= 0.1;
    }
 ...
 */
static int gr__apply_value(const gr_opt *o, const char *val,
                            char *buf, size_t cap, const char *label)
{
    switch (o->kind) {
    case GR_KIND_STR: {
        const char **p = (const char **)o->storage;
        if (!p) { gr__errf(buf, cap, stderr, "internal: null storage"); return GR_ERR; }
        *p = val;
        return GR_OK;
    }
    case GR_KIND_INT:
...
void gr_usage(const gr_spec *spec, FILE *stream)
{
    ...
    for (o = spec->options; o->kind != GR_KIND_END; o++) {
        char tmp[64];
        int  n = 0;
        if (o->short_name)              n += 2;                        /* -x  */
        if (o->short_name && o->long_name) n += 2;                     /* ", " */
        if (o->long_name)               n += 2 + (int)strlen(o->long_name); /* --name */
        if (gr__needs_value(o)) {
            const char *hint = (o->value_hint && *o->value_hint) ? o->value_hint : "VALUE";
            n += 1 + (int)strlen(hint);
        }
        (void)tmp;
        w = (unsigned)n;
    ...
    for (o = spec->options; o->kind != GR_KIND_END; o++) {
        ...
        if (o->short_name)
            len += snprintf(cell + len, sizeof cell - (size_t)len, "-%c", o->short_name);
        if (o->short_name && o->long_name)
            len += snprintf(cell + len, sizeof cell - (size_t)len, ", ");
        if (o->long_name)
            len += snprintf(cell + len, sizeof cell - (size_t)len, "--%s", o->long_name);
        if (gr__needs_value(o)) {
            const char *hint = (o->value_hint && *o->value_hint) ? o->value_hint : "VALUE";
...
int gr_parse(const gr_spec *spec, int argc, char **argv,
             gr_rest *rest, char *errbuf, size_t errcap)
{
    ...
    for (i = 1; i < argc; i++) {
        ...
        if (tok[1] == '-') {
            ...
            if (!o) {
                gr__errf(errbuf, errcap, stderr,
                         "%s: unknown option '--%.*s'", prog, (int)nlen, body);
                return GR_ERR;
            }
            ...
            if (!gr__needs_value(o)) {
                if (eq) {
                    gr__errf(errbuf, errcap, stderr,
                             "%s: option '--%s' does not take a value", prog, o->long_name);
                    return GR_ERR;
                }
                int rc = (o->kind == GR_KIND_FLAG)
                         ? gr__apply_flag (o, errbuf, errcap)
                         : gr__apply_count(o, errbuf, errcap);
                if (rc != GR_OK) return rc;
                continue;
            }
            ...
            if (!val) {
                if (i + 1 >= argc) {
                    gr__errf(errbuf, errcap, stderr,
                             "%s: option '--%s' requires a value", prog, o->long_name);
                    return GR_ERR;
                }
                val = argv[++i];
            }
        ...
        for (const char *p = tok + 1; *p; p++) {
            ...
            if (!o) {
                gr__errf(errbuf, errcap, stderr,
                         "%s: unknown option '-%c'", prog, key);
                return GR_ERR;
            }

            if (o->kind == GR_KIND_FLAG)  { gr__apply_flag (o, errbuf, errcap); continue; }
            if (o->kind == GR_KIND_COUNT) { gr__apply_count(o, errbuf, errcap); continue; }
            ...
            if (!val) {
                if (i + 1 >= argc) {
                    gr__errf(errbuf, errcap, stderr,
                             "%s: option '-%c' requires a value", prog, key);
                    return GR_ERR;
                }
                val = argv[++i];
            }
...
{
    if (app->program && *app->program) return app->program;
    return gr__basename(argv0);
}
 ...
 */
static void gr__path_join(char *dst, size_t cap,
                           const char *prefix, const char *seg)
{
    if (!seg || !*seg) return;
    if (!prefix || !*prefix) snprintf(dst, cap, "%s", seg);
    else                     snprintf(dst, cap, "%s %s", prefix, seg);
}

/**
 * @brief Print a group-level help listing for @p cmds to @p stream.
 */
static void gr__print_group(const gr_app *app, FILE *stream,
                             const gr_cmd *cmds, size_t n,
                             const char *path)
{
    unsigned col = 0, w;
    size_t   i;
    ...
    if (path && *path) fprintf(stream, " %s", path);
                          ...
                          int argc, char **argv)
{
    ...
    return gr__help_walk(prog, app, cmd->children, cmd->child_count,
                          next, cap, argc - 1, argv + 1);
}
                         ...
                         int argc, char **argv)
{
    ...
    if (argc == 0) {
        if (parent) gr__print_cmd(app, stdout, parent, path);
        else {
            fprintf(stdout, "Usage: %s <command> [...]\n\n",
                    app->program ? app->program : "program");
            if (app->blurb) fprintf(stdout, "%s\n\n", app->blurb);
    ...
    if (strcmp(argv[0], "help") == 0 || strcmp(argv[0], "--help") == 0) {
        ...
        char empty[256] = {0};
        return gr__help_walk(prog, app, cmds, n, empty, sizeof empty,
                              argc - 1, argv + 1);
    }
    ...
    /* Descend into group. */
    if (cmd->child_count > 0)
        return gr__dispatch(prog, app, cmd,
                             cmd->children, cmd->child_count,
                             next, argc - 1, argv + 1);
...
int gr_dispatch(const gr_app *app, int argc, char **argv)
{
    ...
    return gr__dispatch(prog, app,
                         NULL, app->commands, app->command_count, "",
                         argc > 1 ? argc - 1 : 0,
                         argc > 1 ? argv + 1 : NULL);
}
