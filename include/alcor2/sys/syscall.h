...
/*
 * Syscall numbers — kernel side of the ABI contract. Linux-compatible values
 * for the POSIX surface (so the forked musl reaches them) plus Alcor-specific
 * extensions. The userland half of this contract lives in the musl fork
 * (arch/x86_64/bits/syscall.h.in → <sys/syscall.h>); keep the two in sync.
 */
...
/** @name Alcor-specific syscalls (reserved band 497–499).
 * @{ */
